...
namespace LogentriesCore.Net
{
    ...
    public class AsyncLogger
    {
        ...
        private static List<Queue<string>> _allQueues = new List<Queue<string>>();
        private static object _queueLock = new object();
        #endregion

        public AsyncLogger()
        {
            Queue = new Queue<string>(QueueSize);
        ...
        protected virtual void Run()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    // Block thread for waiting new elements in queue
                    while (true)
                    {
                        bool shouldBeBlocked = false;
                        lock (_allQueues)
                        {
                            shouldBeBlocked = !Queue.Any();
                        }
                        if(shouldBeBlocked)
                            Thread.Sleep(100);
                        else
